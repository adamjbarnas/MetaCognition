...
export function getTrials(workerId='NA', assignmentId='NA', hitId='NA', dev, reset, numTrials) {
  ...
  $.ajax({
      ...
      data: JSON.stringify({workerId: workerId,image_file: 'meridian_file_list.csv', dev, reset, numTrials }),
...
function runExperiment(trials, workerId, assignmentId, hitId, PORT, FULLSCREEN) {
  ...
  var check_consent = function (elem) {
   ...
   var consent = {
     type: 'external-html',
     url: "./consent_pool.html", //consent_mturk if on mturk; pool if Sona
     cont_btn: "start",
...
//    ]
  ...
  trials.forEach((trial, index) => {
    ...
    images.push('http://'+document.domain+':'+PORT+'/' + trial.image + "/" + trial.objects_image + '.jpg');
    images.push('http://'+document.domain+':'+PORT+'/' + trial.image + "/" + trial.cue_image + '.jpg');
    ...
    let response = {
      ...
      hitId: hitId,
      set: trial.set,
      objs_image: trial.objects_image,
      cue_image: trial.cue_image,
    ...
    const objs_image = trial.objects_image;
    const cue_image = trial.cue_image;
    ...
    // Picture Trial
    let jsPsychTrial = {
      type: "change-detection", // *******changing this to change-detection-splash doesn't work*******

      stimulus: stimulus,
      objects_image: 'http://'+document.domain+':'+PORT+'/' + trial.image + "/" +`${objs_image}.jpg`,
      cue_image: 'http://'+document.domain+':'+PORT+'/' + trial.image + "/" +`${cue_image}.jpg`,
      ...
      objects_interval_duration: 1000,
      cue_interval_duration: 100,
      image_interval_duration: 240,
      blank_objects_interval_duration: 240,
      timeout: 60000,
