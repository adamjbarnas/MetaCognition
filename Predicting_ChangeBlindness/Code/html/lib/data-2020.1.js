...
import { saveData, selectFromArray, shuffle } from './util-2020.1.js';
...
class ExperimentHandler extends PsychObject {
	...
	async save({
		...
		const __worker_ID = ((typeof info.worker_ID === 'string' && info.worker_ID.length > 0) ? info.worker_ID : 'PARTICIPANT');
		...
		if (this._psychoJS.config.experiment.saveFormat === ExperimentHandler.SaveFormat.CSV)
		{
			...
			const key = __worker_ID + '_' + __experimentName + '_' + __datetime;
...
//				offerDataForDownload('results.json', JSON.stringify(documents), 'application/json');
                saveData('results.json', JSON.stringify(documents), 'application/json');
			}
		}
	}
