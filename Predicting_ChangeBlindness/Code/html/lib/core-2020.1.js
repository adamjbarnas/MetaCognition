...
class ServerManager extends PsychObject {
	...
	uploadLog(logs, compressed = false)
	{
		...
		const worker_ID = ((typeof info.worker_ID === 'string' && info.worker_ID.length > 0) ? info.worker_ID : 'PARTICIPANT');
		...
		const filename = worker_ID + '_' + experimentName + '_' + datetime + '.log';
		const data = {
			filename,
			logs,
			compressed
...
class GUI
{
	...
	dialog({
		...
		if (typeof error !== 'undefined')
		{
			...
			htmlCode = '<div id="msgDialog" title="Thanks!">' +
	...
	_onResourceEvents(signal) {
		...
		if (signal.message === ServerManager.Event.RESOURCES_REGISTERED) {
			...
			$("#progressMsg").text('All resources downloaded. Press Ok to continue.');
		...
		else if (signal.message === ServerManager.Event.DOWNLOADING_RESOURCE || signal.message === ServerManager.Event.RESOURCE_DOWNLOADED)
		{
			...
			if (signal.message === ServerManager.Event.RESOURCE_DOWNLOADED)
				$("#progressMsg").text('Downloaded ' + this._progressBarCurrentIncrement/2 + ' / ' + this._progressBarMax/2 + ' resources');
...
class PsychoJS
{
	...
	async quit({ message, isCompleted = false } = {}) {
		...
		try {
            ...
            let participantID = Math.random().toString(20).substr(2, 6) + "iTi" + Math.random().toString(20).substr(2, 6);
			let text = `Thank you for participating! 
            
            <p>

            Your completion code is ${participantID}. 

            <p>

            <b>You must copy the completion code before clicking 'Ok'. If you click 'Ok' without copying the completion code, the completion code will be lost.</b>

            <p>

            Paste the completion code in MTurk to receive payment for the HIT. 
